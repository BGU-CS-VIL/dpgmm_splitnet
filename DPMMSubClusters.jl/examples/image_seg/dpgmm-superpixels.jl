...
for i = 1:x
    for j = 1:y
        input_arr[:, (i-1)*y+j] =
            [img_channels[1, i, j], img_channels[2, i, j], img_channels[3, i, j], i, j]
    end
end
...
#Run the model
labels, clusters, weights =
    DPMMSubClusters.fit(input_arr, hyper_params, 50000.0, iters = 200, verbose = true)
