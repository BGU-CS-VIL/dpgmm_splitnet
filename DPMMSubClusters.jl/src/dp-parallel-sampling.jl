...
Mutates the model.
"""
function init_first_clusters!(dp_model::dp_parallel_sampling, initial_cluster_count::Int64, init_type::String)
    if outlier_mod > 0
        push!(
...
function dp_parallel(
    ...
    outlier_weight = 0,
    outlier_params = nothing,
    init_type_::String = nothing,
)
    global init_type = init_type_
    ...
    init_first_clusters!(dp_model, initial_clusters, init_type)
    if use_verbose
...
function fit(
    ...
    dp_model, iter_count, nmi_score_history, ari_score_history, liklihood_history, cluster_count_history =
        dp_parallel(
            all_data,
            local_hyper_params,
            α_param,
...
dp_model, iter_count , nmi_score_history, liklihood_history, cluster_count_history
 ...
 - `iter_count` Timing for each iteration
 - `nmi_score_history` NMI score per iteration (if gt suppled)
 - `ari_score_history` ARI score per iteration (if gt suppled)
 - `likelihood_history` Log likelihood per iteration.
 - `cluster_count_history` Cluster counts per iteration.
"""
...
function run_model(dp_model, first_iter, model_params = "none", prev_time = 0)
    ...
    for i = first_iter:iterations
        ...
        prev_time = time()
        group_step(dp_model.group, no_more_splits, final, i == 1, init_type)
        ...
        if ground_truth != nothing
            ...
            push!(
                ari_score_history,
                randindex(Int.(ground_truth), group_labels)[1],
            )
        else
            push!(v_score_history, "no gt")
            push!(nmi_score_history, "no gt")
            push!(ari_score_history, "no gt")
        end
        if use_verbose
            ...
            println(
                ...
                " || Log posterior: " *
                "$(@sprintf("%.2f", liklihood_history[end]))" *
                " || Vi score: " *
                "$(@sprintf("%.2f", v_score_history[end]))" *
                " || NMI score: " *
                "$(@sprintf("%.2f", nmi_score_history[end]))" *
                " || ARI score: " *
                "$(@sprintf("%.2f", ari_score_history[end]))" *
                " || Iter Time:" *
                "$(@sprintf("%.2f", iter_time))" *
                " || Total time:" *
                "$(@sprintf("%.2f", sum(iter_count)))" 
            )
        else
    ...
    return dp_model, iter_count, nmi_score_history, ari_score_history, liklihood_history, cluster_count_history
end
...
function plot_group(group::local_group, iter)

    labels = Array(group.labels)
    # labels = Array(group.labels_subcluster)
    pts = Array(group.points)


    min_x, min_y = minimum(pts, dims=2)
    max_x, max_y = maximum(pts, dims=2)
    x = (min_x-2):0.1:(max_x+2)
    y = (min_y-2):0.1:(max_y+2)

    # x = -10:0.1:10
    # y = -10:0.1:10
    # plt = Plots.plot(pts[1,:],pts[2,:], seriestype=:scatter, color = labels, markersize = 0.1)
    
    plt=Plots.plot()
    for (i,cluster) in enumerate(group.local_clusters)
        cp = cluster.cluster_params
        mvn = MvNormal(cp.cluster_params.distribution.μ, cp.cluster_params.distribution.Σ)
        mvnl = MvNormal(cp.cluster_params_l.distribution.μ, cp.cluster_params_l.distribution.Σ)
        mvnr = MvNormal(cp.cluster_params_r.distribution.μ, cp.cluster_params_r.distribution.Σ)
        f(x,y) = pdf(mvn,[x,y])
        fl(x,y) = pdf(mvnl,[x,y])
        fr(x,y) = pdf(mvnr,[x,y])
        # Plots.contour!(x,y,f, levels = [0.0001], linewidth = 2, markercolor = :red, legend = :none,colorbar = false,dpi=150, size=(1200,900))
        contour!(x,y,fr, levels = [0.001], linewidth = 2, linecolor = i, legend = :none, colorbar = false, color = :red,dpi=150, size=(1200,900))
        contour!(x,y,fl, levels = [0.001], linewidth = 2, linecolor = i, legend = :none, colorbar = false)

        # plot_eigenvectors(pts[:,labels .== i], i)

    end

    
    mod_ = 50
    Plots.plot!(pts[1, 1:mod_:end],pts[2, 1:mod_:end], seriestype=:scatter, color = labels[1:mod_:end], markersize = 3, markerstrokewidth = 0.5) #, title="Iteration: " * string(iter) * " | init type: " * string(init_type))
    st = Dates.format(now(), "d.m-HH:MM:SS-")
    return plt
end
